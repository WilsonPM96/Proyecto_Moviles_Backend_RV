...
class Detalle_Reservas : AppCompatActivity() {
   ...
   /* private fun buscarReserva() {
    ...
    class Detalle_Reserva(var idReserva: Int,
                          var idLugar: Int,
                          var estado: Boolean,
                          var fecha: String,
                          var hora_ini: String,
        ...
        override fun writeToParcel(dest: Parcel, flags: Int) = with(dest) {
            writeInt(idReserva)
            writeInt(idLugar)
            writeInt((if (estado) 1 else 0))
            writeString(fecha)
            writeString(hora_ini)
            writeString(hora_fin)
            writeLong(createdAt)
            writeLong(updatedAt)
            writeInt(id)
        }
        ...
        companion object {
            @JvmField
            val CREATOR: Parcelable.Creator<Detalle_Reserva> = object : Parcelable.Creator<Detalle_Reserva> {
                override fun createFromParcel(source: Parcel): Detalle_Reserva = Detalle_Reserva(source)
                override fun newArray(size: Int): Array<Detalle_Reserva?> = arrayOfNulls(size)
            }
        }
    }
}