...
class Lugares : AppCompatActivity() {
    ...
    class Lugar(var idLugar: Int,
                var tipoLugar: Int,
                var ubicacionLugar: String,
                var horarioAtencionLugar: String,
                var createdAt: Long,
                var updatedAt: Long,
                var id: Int,
                var detalles_reservas_lug: List<Detalle_Reservas.Detalle_Reserva>) : Parcelable {
        ...
        override fun writeToParcel(dest: Parcel, flags: Int) = with(dest) {
            writeInt(idLugar)
            writeInt(tipoLugar)
            writeString(ubicacionLugar)
            writeString(horarioAtencionLugar)
            writeLong(createdAt)
            writeLong(updatedAt)
            writeInt(id)
            writeTypedList(detalles_reservas_lug)
        }

        companion object {
            @JvmField
            val CREATOR: Parcelable.Creator<Lugar> = object : Parcelable.Creator<Lugar> {
                override fun createFromParcel(source: Parcel): Lugar = Lugar(source)
                override fun newArray(size: Int): Array<Lugar?> = arrayOfNulls(size)
            }
        }
    }
}